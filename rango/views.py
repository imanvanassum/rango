...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            # Did the user provide a profile picture? If so, we need to get it from the
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        # Use Django's machinery to attempt to see if the username/password combination
        # is valid - a User Object is returned if it is.
        user = authenticate(username=username, password=password)

        # If we have a User object, the details are correct.
        # If None (Python's way of representing the absence of value), no user with matching credentials was found.
        if user:
